export default {
  ...
  AI: Object.freeze([{
    user: /^(唱|播放|音乐|来一?首|歌曲?|听(歌|音乐)?)\s+/,
    sys: ['music-on']
  }, {
    user: /((关[闭掉]?)|停止?|暂停)(音乐|歌曲?)|嘘|静音|安静/,
    sys: ['music-off']
  }, {
    user: /继续(音乐|歌曲?|播放)|(音乐|歌曲?)继续/,
    sys: ['music-goon']
  }, {
    user: /单曲循环/,
    sys: ['music-loop']
  ...
  respone (val) {
    ...
    for (; i < len; i++) {
      if (this.AI[i].user.test(val)) {
        switch (this.AI[i].sys[0]) {
          case 'music-on':
            return ['music', val.replace(this.AI[i].user, '')]
          case 'music-off':
            if (window.Rmusic) window.Rmusic.pause()
            return '[系统回复 ~]: 已关闭音乐'
          case 'music-loop':
            if (window.Rmusic) {
              window.Rmusic.loop = !window.Rmusic.loop
              return '[系统回复 ~]: 已' + (window.Rmusic.loop ? '开启' : '关闭') + '循环'
            }
            return '[系统回复 ~]: 错误，未找到播放器。'
          case 'music-goon':
            if (window.Rmusic) {
